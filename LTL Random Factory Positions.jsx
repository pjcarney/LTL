        ...
        <svg width="1600" height="1000" className="border border-gray-700 rounded">
          ...
          {factories.map(factory => (
            <g key={factory.id}>
              ...
              <text x={factory.x - 20} y={factory.y + 4} textAnchor="end" fill="white" fontSize="12">
                {factory.name}
              </text>
              {/* Order queue visualization */}
              {factory.orderQueue.map((order, index) => (
                <g key={`${factory.id}-order-${index}`}>
                  <text 
                    x={factory.x} 
                    y={factory.y + 35 + (index * 12)} 
                    textAnchor="middle" 
                    fill={order.storeColor} 
                    fontSize="12" 
                    fontWeight="bold"
                  >
                    {order.productType === 'A' ? '👽' : '🦄'}:{order.quantity}
                  </text>
                </g>
              ))}
            </g>
          ...
          {factories.map(factory => (
            factory.currentShipment && (
              <g key={factory.id + '-shipment'}>
                ...
                >
                  {factory.currentShipment.productEmoji}
                </text>
                {/* Pallet count - colored by destination, positioned to the right */}
                <text
                  x={factory.x + 35}
                  y={factory.y}
                  textAnchor="start"
                  fill={factory.currentShipment.color}
                  fontSize="12"
                  fontWeight="bold"
                  dominantBaseline="middle"
                >
